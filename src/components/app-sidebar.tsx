...
import {
  User, Gift, LifeBuoy, Settings, LogOut, FolderKanban, LayoutDashboard,
  Image as ImageIcon, Video, AudioLines, Users, PanelLeft, ChevronDown, Sparkles,
} from 'lucide-react';
...
export function AppSidebar() {
  ...
  const [openSections, setOpenSections] = useState<Record<string, boolean>>({ Gallery: true });
      ...
      >
        ...
        <div className="flex items-center gap-3 px-3 pt-4 pb-3">
          ...
          <div className="flex min-w-0 flex-1 flex-col">
            <span className="truncate text-sm font-bold leading-tight" style={{ color: 'var(--brut-text-heading)' }}>
              ClipFlow
            </span>
            <span className="truncate text-xs leading-tight" style={{ color: 'var(--brut-text-muted)' }}>
              AI Studio
            </span>
          </div>
          <button className="flex h-6 w-6 items-center justify-center rounded" aria-label="Switch account" style={{ color: 'var(--brut-text-muted)' }}>
            <ChevronDown className="h-4 w-4" />
          </button>
        </div>

        <div className="border-y-2 px-3 py-4" style={{ borderColor: 'var(--brut-border)', backgroundColor: 'var(--brut-bg)' }}>
          <div className="flex gap-2">
            {quickActions.map((action) => (
              <Link
                ...
                style={{ borderColor: 'var(--brut-border)', borderRadius: 'var(--brut-radius-base)', backgroundColor: 'var(--brut-bg)', boxShadow: 'var(--brut-shadow-xs)' }}
              >
                <action.icon className="h-4 w-4" style={{ color: 'var(--brut-text-heading)' }} />
                <span className="text-[11px] font-bold leading-tight" style={{ color: 'var(--brut-text-heading)' }}>{action.label}</span>
              </Link>
            ))}
          </div>
        </div>
              ...
              if (item.children) {
                return (
                  <li key={item.label}>
                    ...
                    >
                      <Icon className="h-5 w-5 shrink-0" style={{ color: active ? 'var(--brut-text-heading)' : 'var(--brut-text-body)' }} />
                      <span className="flex-1 text-left">{item.label}</span>
                      <ChevronDown className="h-4 w-4 transition-transform" style={{ transform: expanded ? 'rotate(180deg)' : 'rotate(0deg)', color: 'var(--brut-text-muted)' }} />
                    </button>
                    {expanded && (
                          ...
                          <li key={child.href}>
                            <Link
                              ...
                              style={{
                                paddingLeft: '44px',
                                borderRadius: 'var(--brut-radius-base)',
                                color: isActive(child.href) ? 'var(--brut-fg-brand-strong)' : 'var(--brut-text-body)',
                                fontWeight: isActive(child.href) ? '700' : '500',
                                backgroundColor: isActive(child.href) ? 'var(--brut-bg-brand-soft)' : 'transparent',
                              }}
            ...
            {bottomItems.map((item) => (
              <li key={item.label}>
                <Link
                  href={item.href}
                  className="flex items-center gap-3 rounded-md px-2 py-2 text-sm font-bold transition-colors"
                  style={{ borderRadius: 'var(--brut-radius-base)', color: (item as any).danger ? 'var(--brut-danger)' : 'var(--brut-text-heading)' }}
                >
                  <item.icon className="h-5 w-5 shrink-0" style={{ color: (item as any).danger ? 'var(--brut-danger)' : 'var(--brut-text-body)' }} />
                  <span>{item.label}</span>
                </Link>
              </li>
            ))}
