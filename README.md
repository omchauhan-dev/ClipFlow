...
Only **1 GPU** runs at a time. When you send 5 images at once, they queue on the same GPU:
...
### Config (in `comfyui_modal.py`)

| Setting | Value | Effect |
|---------|-------|--------|
| `max_containers` | 1 | Uses only one GPU at a time |
| `scaledown_window` | 300s | GPU shuts down after 5 min idle |
| `min_containers` | 0 | No GPU running when idle |
